...
Ellipse::Ellipse(const int &x, const int &y, const uint &width, const uint &height)
{
    this->x=x;
    this->y=y;
    this->width=width;
...
void Ellipse::paintEvent(QPainter &painter)
{
    int dx = getDisplacement().x();
    int dy = getDisplacement().y();
    painter.drawEllipse(x+dx,y+dy, width, height);
}
